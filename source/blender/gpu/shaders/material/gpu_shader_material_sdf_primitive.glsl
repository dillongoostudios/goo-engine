...
float sdf_2d_square_line(vec2 p, vec2 a, vec2 b, float lw)
{
  float l = length(b - a);
  vec2 d = (b - a) / l;
  vec2 q = (p - (a + b) * 0.5);
  q = mat2(d.x, -d.y, d.y, d.x) * q;
  q = abs(q) - vec2(l, lw) * 0.5;
...
float sdf_2d_parabola(vec2 pos, float k)
{
  ...
  if (k != 0.0) {
    ...
    float x = (h > 0.0) ? pow(-q + r, 1.0 / 3.0) - pow(abs(-q - r), 1.0 / 3.0) * sign(q + r) :
                          2.0 * cos(atan(r, -q) / 3.0) * sqrt(-p);

    d = length(pos - vec2(x, k * x * x)) * sign(pos.x - x);
  }
...
float sdf_2d_heart(vec2 p, float r)
{
  ...
  if (p.y + p.x > r) {
    return sqrt(dot2(p - vec2(0.25, 0.75) * r)) - M_SQRT2 / 4.0 * r;
  }
  else {
    return sqrt(min(dot2(p - vec2(0.0, 1.0) * r), dot2(p - 0.5 * max(p.x + p.y, 0.0)))) *
           sign(p.x - p.y);
  }
}
...
float sdf_2d_vesica_orig(vec2 p, float r, float d)
{
  p = abs(p);
...
float sdf_2d_vesica_alt(vec2 p, float r, float d)
{
  p = abs(p);
  float b = (r * r - d * d);
  vec2 a = vec2(p.y * d, p.x + d);
  a *= a * sign(a);
  return ((a.x / b > a.y)) ? length(p - vec2(0.0, sqrt(b))) * sign(d) :
                             length(p + vec2(d, 0.0)) - r;
}
...
{
  vec2 cs = cosin(clamp(a * 0.5, 0.0, M_PI));
  p.x = abs(p.x);
  float l = length(p);
  p = mat2(-cs.x, cs.y, cs.y, cs.x) * p;
  p = vec2((p.y > 0.0 || p.x > 0.0) ? p.x : l * sign(-cs.x), (p.x > 0.0) ? p.y : l);
