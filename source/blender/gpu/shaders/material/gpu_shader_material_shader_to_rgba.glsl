...
void node_shader_to_rgba(Closure cl, out vec4 outcol, out float outalpha)
{
  ...
  vec4 spec_cl = vec4(0.0, 0.0, 0.0, 1.0);
  vec4 diff_cl;
  unpackVec4(cl.ssr_data, spec_cl, diff_cl);
  ...
  if (ssrToggle && FLAG_TEST(cl.flag, CLOSURE_SSR_FLAG)) {
    ...
    in_Glossy_0.roughness = spec_cl.a;
  ...
  outcol = vec4((spec_accum.rgb * spec_cl.rgb) + cl.radiance, 1.0);
