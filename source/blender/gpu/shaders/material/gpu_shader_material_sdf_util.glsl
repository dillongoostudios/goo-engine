...
vec3 pModMirror3(inout vec3 p, vec3 size)
{
  vec3 halfsize = size * 0.5;
  vec3 c = floor(safe_divide((p + halfsize), size));
  p = safe_mod(p + halfsize, size) - halfsize;
  p *= safe_mod(c, vec3(2.0)) * 2.0 - vec3(1.0);
  return c;
}
...
vec2 pModGrid2(inout vec2 p, vec2 size)
{
  ...
  p *= safe_mod(c, vec2(2.0)) * 2.0 - vec2(1.0);
  p -= size / 2.0;
  if (p.x > p.y) {
    p.xy = p.yx;
  }
