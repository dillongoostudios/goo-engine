...
static bNodeSocketTemplate sh_node_sdf_op_in[] = {
    {SOCK_FLOAT, N_("Distance"), 0.5f, 0.5f, 0.5f, 1.0f, -10000.0f, 10000.0f, PROP_NONE},
    {SOCK_FLOAT, N_("Distance"), 0.5f, 0.5f, 0.5f, 1.0f, -10000.0f, 10000.0f, PROP_NONE},
    {SOCK_FLOAT, N_("Value"), 0.0f, 0.5f, 0.5f, 1.0f, -10000.0f, 10000.0f, PROP_NONE},
    {SOCK_FLOAT, N_("Value"), 0.0f, 0.5f, 0.5f, 1.0f, -10000.0f, 10000.0f, PROP_NONE},
    {SOCK_INT, N_("Count"), 1.0f, 0.0f, 0.0f, 1.0f, -10000.0f, 10000.0f, PROP_NONE},
    {-1, ""}};

static bNodeSocketTemplate sh_node_sdf_op_out[] = {{SOCK_FLOAT, N_("Distance")}, {-1, ""}};
...
static void node_shader_label_sdf_op(bNodeTree *UNUSED(ntree),
                                     bNode *node,
                                     char *label,
                                     int maxlen)
{
...
static void node_shader_update_sdf_op(bNodeTree *UNUSED(ntree), bNode *node)
{
  NodeSdfOp *sdf = (NodeSdfOp *)node->storage;
  ...
  /* Distance */
  nodeSetSocketAvailability(sockInputA, true);

  nodeSetSocketAvailability(sockInputB,
                            !ELEM(sdf->operation,
                                  SHD_SDF_OP_MASK,
                                  SHD_SDF_OP_INVERT,
                                  SHD_SDF_OP_DILATE,
  ...
  nodeSetSocketAvailability(sockCount, ELEM(sdf->operation, SHD_SDF_OP_ONION, TEST_SDF));
...
static void node_shader_init_sdf_op(bNodeTree *UNUSED(ntree), bNode *node)
{
...
static void node_shader_buts_sdf_op(uiLayout *layout, bContext *UNUSED(C), PointerRNA *ptr)
{
  uiItemR(layout, ptr, "operation", 0, "", ICON_NONE);
  int type = RNA_enum_get(ptr, "operation");
  if (ELEM(type, SHD_SDF_OP_MASK, TEST_SDF)) {
...
void register_node_type_sh_sdf_op(void)
{
  ...
  node_type_init(&ntype, node_shader_init_sdf_op);
  node_type_label(&ntype, node_shader_label_sdf_op);
  node_type_update(&ntype, node_shader_update_sdf_op);
